...
class ListaLinkata
{
	...
	public:
		ListaLinkata() { Testa = Coda = Corrente = Prev = NULL ; Size = 0 ; Position = -1; };
		...
		void reset ( int );								// aggiorna i puntatori Corrente e Prev
...
template <class T>
Nodo<T>* ListaLinkata<T>::creaNodo( const T& item , Nodo <T>* ptrnext = NULL )
{
	Nodo<T>* nuovo;
	nuovo = new Nodo<T>( item , ptrnext );
...
void ListaLinkata<T>::copiaLista( const ListaLinkata<T>& list )
{
	...
	// se la lista è vuota la funzione termina:
	if ( Position == -1 )
		return;
...
{
	// verifica che la lista passata come parametro NON è quella chiamante:
	if ( this == &lista )
		return *this;
...
void ListaLinkata<T>::reset( int pos = 0 )
{
	...
	if ( Testa == NULL )
		return;
	// se la posizione non è valida, il programma termina:
	if ( pos < 0 || pos >= Size )
		return;
...
{
	// posizionamento alla testa della lista se questa non è vuota:
	if ( Testa != NULL )
		reset();
